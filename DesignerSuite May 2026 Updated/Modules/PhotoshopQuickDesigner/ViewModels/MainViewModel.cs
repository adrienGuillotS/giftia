...
using Photoshop;
using PhotoshopQuickDesigner.Module.Helpers;
...
namespace PSQuickDesigner.ViewModels
{
    ...
    {
        private static string processedZipFilesPath = "";
        private static string extractPath = "";

        #region Binding properties
        private float _smallImageWidthInCentimeters = 10;
        public float SmallImageWidth
        {
            get { return _smallImageWidthInCentimeters; }
            set
            {
                _smallImageWidthInCentimeters = value;
                OnPropertyChanged(nameof(SmallImageWidth));
            }
        }

        private float _smallImageHeightInCentimeters = 10;
        public float SmallImageHeight
        {
            get { return _smallImageHeightInCentimeters; }
            set
            {
                _smallImageHeightInCentimeters = value;
                OnPropertyChanged(nameof(SmallImageHeight));
            }
        }
        ...
        private CreateNewPresetViewModel _createNewPresetViewModel;

        public CreateNewPresetViewModel CreateNewPresetViewModel
        {
            get { return _createNewPresetViewModel; }
            set
            {
                if (_createNewPresetViewModel != value)
                {
                    _createNewPresetViewModel = value;
                    OnPropertyChanged(nameof(CreateNewPresetViewModel));
        ...
        public void StartProcessing(string zipFolderPath, WorkingModeEnum workingMode = WorkingModeEnum.Amazon)
        {
            processedZipFilesPath = Path.Combine(zipFolderPath, "processed");
            extractPath = Path.Combine(processedZipFilesPath, "temp");

            var processedDocNames = new List<string>();
            var processedZipFiles = new List<string>();

            string[] zipFiles = ZipFileHelper.GetZipFiles(zipFolderPath, processedZipFilesPath);
            ...
            foreach (string zipFile in zipFiles)
            {
                try
                {
                    ...
                    {
                        ProcessAmazonZip(zipFile, extractPath, zipFolderPath, processedDocNames, workingMode);
                    }
                    else // TEMU
                    {
                        ProcessTemuZip(zipFile, extractPath, zipFolderPath, processedDocNames, workingMode);
                    }

                    processedZipFiles.Add(zipFile);
                }
                catch (Exception ex)
                {
            ...
            // Move processed zip files
            if (processedZipFiles.Count > 0)
                ZipFileHelper.MoveZipFiles(processedZipFiles, processedZipFilesPath);
        }

        private void ProcessAmazonZip(string zipFile, string extractPath, string zipFolderPath, List<string> processedDocNames, WorkingModeEnum workingMode)
        {
            var xmlFiles = ZipFileHelper.GetXmlFilesFromZip(zipFile, extractPath);
            if (xmlFiles == null || xmlFiles.Count == 0)
                return;

            string pattern1 = @"^\d{3}-\d{7}-\d{7}(-\d+)?$";
            string pattern2 = @"^(\d+)-(\d{3}-\d{7}-\d{7})(-\d+)?$";
            foreach (string xmlFile in xmlFiles)
            {
                List<OrderDocuement> documents = XmlFileHelper.ReadXmlFile(xmlFile);

                foreach (var doc in documents)
                {
                    doc.DocumentName = EnsureUniqueName(doc.DocumentName, processedDocNames, pattern1, pattern2, workingMode);
                    processedDocNames.Add(doc.DocumentName);
                }

                CreatePhotoshopDocument(documents, zipFolderPath, extractPath);
            }
        }

        private void ProcessTemuZip(string zipFile, string extractPath, string zipFolderPath, List<string> processedDocNames, WorkingModeEnum workingMode)
        {
            string pattern1 = @"^\d{3}-\d{17}$";
            string pattern2 = @"^(\d{3}-\d{17}-)(\d+)?$";
            List<OrderDocuement> documents = TemuOrderFileHelper.ExtractImages(zipFile, extractPath);
        ...
        private string EnsureUniqueName(string name, List<string> existingNames, string pattern1, string pattern2, WorkingModeEnum workingMode)
        {
            if (!existingNames.Any(x => x.Equals(name, StringComparison.OrdinalIgnoreCase)))
                return name;

            // Pattern 1
            if (ValidatePattern(name, pattern1) || ValidatePattern(name, @"^\d+$"))
            {
                int i = 2;
                string newName = $"{name}-{i}";

                while (existingNames.Any(x => x.Equals(newName, StringComparison.OrdinalIgnoreCase)))
                    newName = $"{name}-{++i}";
            ...
            {
                string newName = ValidateAndIncrementTemu(name, pattern);
                while (existingNames.Any(x => x.Equals(newName, StringComparison.OrdinalIgnoreCase)))
                    newName = ValidateAndIncrementTemu(newName, pattern);
                return newName;
            }

            return workingMode == WorkingModeEnum.Amazon ? ValidateAndIncrement(name, pattern2) :
                ValidateAndIncrementTemu(name, pattern2);
        }

        private static string IncreamentPostfix(string name, string pattern)
        {
            ...
            if (match.Success)
            {
                ...
                int incrementedPostfix = int.Parse(postFix) + 1;

                // Construct the new name
                return $"{namePattern}-{incrementedPostfix}";
            }
        ...
        static string ValidateAndIncrementTemu(string input, string pattern)
        {
            ...
            if (match.Success)
            {
                ...
                // Increment the prefix
                int incrementedPrefix = int.Parse(postFix) + 1;

                // Construct the new name
                return $"{namePattern}{incrementedPrefix}";
            }
            else
            {
                Debug.WriteLine($"Pattern NOT valid for: {input}");
                return input;
            }
        }

        static string ValidateAndIncrement(string input, string pattern)
        {
            Regex regex = new Regex(pattern);
            Match match = regex.Match(input);
            ...
            if (match.Success)
            {
                ...
                string restOfPattern = match.Groups[2].Value;
                string additionalNumericPart = match.Groups[3].Value;

                // Increment the prefix
                int incrementedPrefix = int.Parse(prefix) + 1;

                // Construct the new name
                return $"{incrementedPrefix}-{restOfPattern}{additionalNumericPart}"; ;
            }
            else
            {
                Debug.WriteLine($"Pattern NOT valid for: {input}");
        ...
        private void CreatePhotoshopDocument(List<OrderDocuement> documents, string zipFolderPath, string extractPath)
        {
            foreach (var item in documents)
            {
                if (item.MainImage == null || string.IsNullOrWhiteSpace(item.MainImage))
                {
                    item.MainImage = documents.FirstOrDefault(a => a.MainImage != null
                    && !string.IsNullOrWhiteSpace(a.MainImage))?.MainImage;
                }
                if (item.MainImage != null)
                    item.MainImage = System.IO.Path.Combine(extractPath, item.MainImage);
                if (item.SmallImage != null)
                    item.SmallImage = System.IO.Path.Combine(extractPath, item.SmallImage);

                PhotoshopHelper._smallImageWidthInCM = SmallImageWidth;
                PhotoshopHelper._smallImageHeightInCM = SmallImageHeight;
                PhotoshopHelper._psResolutions = SelectedResolutionUnit.Value;

                PhotoshopHelper.SetPsUnits(SelectedPreset.Units);

                PhotoshopHelper.ImportImage(item.MainImage, item.SmallImage,
                       item.Text, item.FontFamily, 120, item.FontColorHex, SelectedPreset.Width,
                       SelectedPreset.Height, zipFolderPath, item.DocumentName,
                       SelectedPreset.Resolution,
                       PhotoshopHelper.GetPsNewDocumentMode(SelectedPreset.Mode),
                       PhotoshopHelper.GetFillColor(SelectedPreset.Fill.ToString()),
                       SelectedPreset.Scale, SelectedPreset.Depth,
