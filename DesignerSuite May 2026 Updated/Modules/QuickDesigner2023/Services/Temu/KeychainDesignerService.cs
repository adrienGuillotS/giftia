    ...
    public class KeychainDesignerService : CoreldrawDesignerService
    {
        ...
        public override double ImportImageDataIntoCDR(Corel.Interop.VGCore.Layer layer, string destinationFolderPath, ICsvRecord csvPositioningData, string zipFilePath, double positionY)
        {
            ...
            if (csvPositioningData is KeychainCsvRecord position)
            {
                ...
                IImageDataExtractorService imageDataExtractorService = new TemuKeychainImageDataExtractorService();
                var data = (KeychainOrderDataItem)imageDataExtractorService.ExtractImages(zipFilePath, extractPath);
                ...
                var imageShape = ImportMainImage(layer, data.ImageName, extractPath, position);

                ImportSnapshotImage(layer, data.SnapshotImageName, extractPath, position);
        ...
        private void ImportSnapshotImage(Layer layer, string imageName, string destinationPath, KeychainCsvRecord position)
        {
            if (string.IsNullOrWhiteSpace(imageName)) return;
            // snapshot
            var snapPath = Path.Combine(destinationPath, imageName);
        ...
        private Shape? ImportMainImage(Layer layer, string imageName, string destinationPath, KeychainCsvRecord position)
        {
            if (string.IsNullOrWhiteSpace(imageName)) return null;
            // main image
            var mainImagePath = Path.Combine(destinationPath, imageName);
        ...
        private void CreateTextElement(Layer layer, KeychainOrderDataItem data, KeychainCsvRecord position)
        {
            Shape box = CreateEmptyRectangle(position);
            Shape? icon = null;
            ...
            var cleanText = data.InputValue != null
                ? EscapeNewlineRegex.Replace(data.InputValue, "\r\n")
                : string.Empty;
